...
import { listLanes, saveLane, restartBackend } from "@/lair/api";
  ...
  return (
    <section className="flex flex-col gap-3 p-4">
      ...
      <div className="flex items-center gap-2 rounded-md border border-border bg-card/60 px-3 py-2">
        ...
        <button
          type="button"
          onClick={() => void restartBackend("uniclaude-proxy")}
          className="ml-auto rounded-md border border-border px-2 py-0.5 text-[10.5px] text-muted-foreground hover:bg-muted hover:text-foreground"
        >
          restart
        </button>
      </div>
